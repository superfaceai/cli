...
export const DEFAULT_CONTEXT_DIR = join(DEFAULT_SUPERFACE_DIR, 'context');
...
export function buildProfilePath(profileId: string): string {
  return join(DEFAULT_SUPERFACE_DIR, `${profileId}${PROFILE_EXTENSION}`);
}
...
export function buildProviderPath(providerName: string): string {
  return join(DEFAULT_SUPERFACE_DIR, `${providerName}${PROVIDER_EXTENSION}`);
}
...
export function buildIndexPath(providerName: string): string {
  return join(DEFAULT_CONTEXT_DIR, `${providerName}${INDEX_EXTENSION}`);
}
